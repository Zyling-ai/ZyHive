...
[![Version](https://img.shields.io/badge/version-26.3.17-brightgreen.svg)](CHANGELOG.md)
...
| 26.3.17 | **稳定性修复**：新实例登录 401 死循环、update check 未登录触发问题 | ✅ |
| 26.3.17 | **Windows 安装脚本双修**：irm\|iex 崩溃、服务注册失败 | ✅ |
| 26.3.17 | **Provider 测试修复**：MiniMax 等厂商"未配置调用地址"问题 | ✅ |
| 26.3.17 | **MiniMax 探测修复**：改用 chat completion 探测替代不支持的 GET /v1/models | ✅ |
| 26.3.17 | **安装向导**：bash / PowerShell 交互式向导（8 provider 预设） | ✅ |
| 26.3.17 | **工具调用修复**：Runner 三个路径补全 SupportsTools 字段 | ✅ |
| 26.3.17 | **agent_list 修复**：Pool 正式注册 WithAgentLister | ✅ |
| 26.3.17 | **工具生态全面升级**：web_search / image / process / cron_* / sessions_* / acp_* + 工具权限系统 + 内置心跳 + ACP 编程代理 | ✅ |
