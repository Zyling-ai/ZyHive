...
[![Version](https://img.shields.io/badge/version-26.7.31v8-brightgreen.svg)](CHANGELOG.md)
...
> **发布状态**
> 当前公开稳定版为 `26.7.31v8`，正式 Release 必须通过全新安装、服务启动、鉴权、基础 API、成员与文件持久化和旧版更新全流程测试。
...
- 每个来源 24 小时内最多 8 个会话；
- 全局最多 4 个公共模型任务和 32 条公共 SSE 连接；
- 单条消息最多 16 KiB，单次模型执行最多 2 分钟；
- `web_fetch` 和浏览器初始导航拒绝本机、内网、链路本地、云元数据地址及危险 DNS 解析。

可通过 `ZYHIVE_PUBLIC_REQUESTS_PER_MINUTE`、`ZYHIVE_PUBLIC_RATE_PER_MINUTE`、`ZYHIVE_PUBLIC_MAX_SESSIONS`、`ZYHIVE_PUBLIC_MAX_CONCURRENT`、`ZYHIVE_PUBLIC_MAX_SSE`、`ZYHIVE_PUBLIC_MAX_MESSAGE_BYTES`、`ZYHIVE_PUBLIC_RUN_TIMEOUT` 和 `ZYHIVE_PUBLIC_SESSION_TTL` 调整。仅在服务明确位于可信反向代理后时设置 `ZYHIVE_TRUST_PROXY_HEADERS=1`。
