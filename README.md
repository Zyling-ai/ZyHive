...
[![Version](https://img.shields.io/badge/version-26.8.1v6-brightgreen.svg)](CHANGELOG.md)
...
> **发布状态**
> 当前公开稳定版为 `26.8.1v6`，正式 Release 必须通过全新安装、服务启动、鉴权、基础 API、成员与文件持久化、旧版更新和更新后健康回滚全流程测试。
...
- Ollama（仅允许显式配置的本机精确回环端点，无需 API Key）
...
公开 Web 渠道默认启用以下服务端硬限制，无需额外配置：

- 每个来源每分钟最多 60 次公共接口请求、12 次模型执行；
- 每个来源 24 小时内最多 8 个会话；
- 全局最多 100 个 30 分钟活跃会话、4 个公共模型任务和 32 条公共 SSE 连接；
- 同一会话只允许一个任务运行，繁忙时拒绝新请求；
- 单条消息最多 16 KiB，单次模型执行最多 2 分钟；
- `web_fetch`、Chromium 全部流量及模型动态 BaseURL 拒绝本机、内网、链路本地、云元数据地址及危险 DNS 解析；Ollama 仅开放显式配置的本机精确端口。

可通过 `ZYHIVE_PUBLIC_REQUESTS_PER_MINUTE`、`ZYHIVE_PUBLIC_RATE_PER_MINUTE`、`ZYHIVE_PUBLIC_MAX_SESSIONS`、`ZYHIVE_PUBLIC_MAX_ACTIVE_SESSIONS`、`ZYHIVE_PUBLIC_MAX_CONCURRENT`、`ZYHIVE_PUBLIC_MAX_SSE`、`ZYHIVE_PUBLIC_MAX_MESSAGE_BYTES`、`ZYHIVE_PUBLIC_RUN_TIMEOUT`、`ZYHIVE_PUBLIC_SESSION_TTL` 和 `ZYHIVE_PUBLIC_ACTIVE_SESSION_TTL` 调整。仅在服务明确位于可信反向代理后时设置 `ZYHIVE_TRUST_PROXY_HEADERS=1`。
...
> 历史记录：`26.5.10v6` 至 `26.5.10v24` 期间曾完成 19 次 staging 部署及对应 smoke；这不代表当前 `26.8.1v6` 仍运行在该 staging 环境。
