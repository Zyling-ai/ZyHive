...
- Cron 为每个计划 occurrence 原子持久化 claim，多进程只执行一次，中断副作用标记为“待确认”且不自动重放；
- 审批 SSE 不再长期占用共享配置读锁，设置页在事件流连接期间也能立即保存并正确提示待重启状态；
- 正式发布改为 Draft-first 状态机：四平台安装更新、SBOM、签名、provenance 和可复现构建全部通过后才唯一公开。

## [26.8.1v14] — 2026-08-01 · 🧾 可验证软件供应链
...
- 供应链工作流会从精确 Tag 重建管理界面与 Go 二进制，并逐字节比对已发布资产；
- 本地与云端统一使用 `-buildvcs=false`，排除 Tag 创建时序造成的非功能性二进制差异；
- 上传后重新下载并验证摘要、签名身份和构建来源，任一步失败即撤回 Release；
- 安装器支持 `--verify-signature` 严格模式，缺失 cosign、bundle 或身份不匹配时拒绝安装。

## [26.8.1v13] — 2026-08-01 · ⚠️ 已撤回

- 前端重建已对齐，但门禁继续发现创建 Tag 前后的 Go VCS 构建元数据会改变二进制；
- 自动撤回逻辑再次按设计生效，未绕过逐字节比对；
- 修复后的可复现构建与完整供应链顺延至 `26.8.1v14`。

## [26.8.1v12] — 2026-08-01 · ⚠️ 已撤回

- 首次供应链门禁发现本地发布先重建管理界面、云端精确重建仅编译 Go，导致字节不一致；
- 自动撤回逻辑按设计生效，未将缺少完整可复现证据的版本保留为稳定版；
- 不复用已发布 Tag，前端重建修复进入 `26.8.1v13`。

## [26.8.1v11] — 2026-08-01 · 💾 安全完整备份恢复

- 新增版本化 `tar.gz` 备份格式，manifest 记录路径、类型、权限、大小和 SHA-256；
- 覆盖配置、成员、工作区、会话、项目、Cron、Goal、任务及运行记录；
- 新增 `zyhive backup create|inspect|restore` 非交互命令和 `--no-service` 运维模式；
- 创建阶段拒绝符号链接、特殊文件和递归包含输出，恢复前完整校验所有条目；
- 恢复使用隔离暂存、原子替换和失败回滚，拒绝路径穿越、重复路径、损坏及不完整归档；
- Release E2E 会先破坏持久数据，再恢复、重启并逐项复核核心对象。

## [26.8.1v10] — 2026-08-01 · 🧭 核心用户旅程门禁

- Release E2E 新增带 Provider 与默认模型的首次设置、随机令牌和重复安装保持验证；
- 增加 Web 渠道密码、公开对话、禁用语义和服务重启保持验证；
- 使用隔离的确定性模型服务验证真实流式对话和后台任务结果；
- 增加成员工作区、项目、Goal、里程碑、Cron 与任务记录的跨重启一致性验证；
- 所有测试仅使用临时目录、随机端口和本地模拟模型，不读取用户数据或真实凭据。

## [26.8.1v9] — 2026-08-01 · 🔒 配置与 API 安全收口

- CORS 默认限制为同源，可通过 `gateway.cors.allowedOrigins` 或精确 `publicUrl` 显式授权远程管理界面；
- 配置更新改为递归合并，修改端口不再清空 `bind`、`publicUrl` 等同级字段；
- 配置读取统一遮蔽 Provider、Model、Channel、Tool 和管理令牌等秘密；
- Cron 拒绝不存在的成员，Agent 拒绝未知模型并支持创建时原子保存工具策略；
- Model 创建强制校验必填字段、Provider 引用和类型一致性；
- Telegram、飞书渠道测试改为真实探测，未实现渠道不再返回假成功；
- Telegram 安装向导修正白名单字段位置，并新增配置、安全和脚本回归测试。

## [26.8.1v8] — 2026-08-01 · 🌐 远程管理连接统一

- REST、对话流、断线续传、任务轮询、审批流、工具审计、头像、文件上传和更新轮询统一读取同一个远程服务器地址；
- API 客户端改为每次请求动态读取服务器地址，修复登录页保存远程地址后仍向当前站点发送部分请求的问题；
- 审批 EventSource 继续只携带短期单次凭证，同时正确连接远程实例，不在 URL 中暴露管理员令牌；
- 修复派遣产物读取使用错误令牌键的问题，统一使用 `aipanel_token`；
- 新增远程地址规范化、运行期切换、Axios 路由和审批流回归测试，并同步嵌入式管理界面。

## [26.8.1v7] — 2026-08-01 · ♻️ 更新后健康确认与自动回滚

- 在线更新在替换前写入权限为 `0600` 的待确认记录；记录或独立守护进程无法建立时拒绝进入无保护的新版本；
- 旧版 `.bak` 以脱离服务进程组的守护模式验证新进程 `/healthz`，只有状态正常且版本精确匹配才确认更新；
- 新进程启动失败、健康超时或版本错误时，守护进程原子恢复旧二进制并终止失败进程，由 systemd/launchd 重启旧版本；
- 新旧进程通过待确认记录交接 PID 和更新状态，回滚结果会在旧版本恢复后通过更新状态接口显示；
- 更新入口的并发占位改为锁内完成，避免两个请求同时启动替换任务；版本状态读写同步消除数据竞争；
- Release E2E 新增真实发布二进制的健康确认和失联回滚测试，覆盖记录权限、路径关系、一次性令牌和恢复结果；
- 发布后验证改为通过鉴权 API 查询精确标签，不再依赖可能受节点缓存影响的 GitHub `latest` 响应。

## [26.8.1v6] — 2026-08-01 · ⚠️ 已撤回

- 首次发布后的 macOS 门禁因 GitHub `latest` 节点缓存误判而撤回；
- 修复门禁后曾错误复用相同标签，导致下载缓存中的旧二进制与新校验和不一致，门禁再次正确阻断安装；
- 该版本不作为公开稳定版且不得重新发布，完整修复改用全新不可变标签 `26.8.1v7`。

## [26.8.1v5] — 2026-08-01 · 📚 文档事实治理与安装边界修正

- 全量复核现行、历史、提案、实验和生成记录文档，补齐状态、时间、适用范围与事实优先级；
- 修正 README 的工具名称、飞书能力、升级阶段、版本里程碑和 aiteam 启用边界，动态能力不再写成固定长期数字；
- 归档已落地提案和历史 Bug 状态，清除旧项目 README 中已废止的安装、认证和商业许可表述；
- 更新优化路线图与进度表，明确已完成、部分完成和剩余任务，不再把历史缺陷清单解释为当前全部待办；
- 安装入口统一使用 `curl -fsSL`，HTTP 错误不再静默进入 shell；
- 安装脚本明确拒绝没有正式发布产物的 32 位 ARM（armv6/armv7），避免映射到不存在的下载文件。

## [26.8.1v4] — 2026-08-01 · ⏱️ Cron 一次性、时区与重叠执行修复

- `at` 不再误当 Cron 表达式：一次性时间会规范为绝对 UTC，准时执行一次后自动禁用，重启不会重复触发；
- Cron 表达式真实应用任务时区，兼容 5/6 字段格式；无效表达式、时区和过去时间在保存前直接拒绝；
- 同一任务同时只允许一个运行，定时触发或手动触发重叠时按默认 `skip` 策略记录为 `skipped`，API 返回 409；
- 新增运行上下文所有权，删除任务或关闭服务会取消正在执行的 Cron，服务退出会等待运行完整回收；
- 新增原子注册、更新、删除和 `jobs.json` 临时文件替换，调度或持久化失败不再留下内存脏任务；
- 成员侧 `cron_list` / `cron_remove` 强制限制为当前成员，禁止通过 `all` 或任务 ID 查看、删除其他成员任务；
- Cron 管理页可直接创建周期表达式、固定间隔和一次性任务，并区分成功、失败和跳过记录。

## [26.8.1v3] — 2026-08-01 · 🛡️ 渠道/更新/媒体剩余动态出站收口

- 飞书 HTTP 客户端、头像下载、探测接口与工具侧媒体请求统一接入 `netguard`；WebSocket 长连接先做公网校验，再限制为 Feishu/Lark 官方主机；
- Telegram Bot API、文件下载与探测客户端关闭环境代理，并在拨号层拒绝私网、元数据和 DNS 重绑定；
- 在线更新的版本查询、校验和下载、安装包拉取与可达性探测全部改用安全客户端；
- Brave `web_search`、图片 URL 拉取、AI Panel Provider 测试与公网 IP 探测、团队 FX 汇率请求统一走 `netguard`；
- 图片下载增加 HTTP 状态校验和 20 MiB 响应体上限；飞书头像 URL 在 GET 前显式校验；
- 新增 WebSocket 校验、飞书主机白名单和更新下载安全客户端回归测试。
...
- `process` 的列表、日志、标准输入、轮询和终止操作全部校验所有者，跨成员或跨会话访问统一返回不存在；
- 每个所有者最多同时运行 4 个后台进程，全局最多 32 个；已完成记录按每个所有者 16 个、全局 128 个自动裁剪；
- 标准输出与错误输出分别限制为 1 MiB，单次后台任务最长运行 30 分钟，完成记录 10 分钟后自动清理；
- Linux、macOS 超时或取消时终止完整进程组，Windows 使用任务树终止，服务退出时回收全部托管子进程；
- ACP 工作目录强制限制在成员工作区，继承环境先移除 API Key、Token、密码等敏感变量，再注入 ACP 显式配置；
- Heartbeat 禁止同一成员重叠执行，非会话任务使用独立运行身份；前台 Bash 同步补齐调用取消和进程组回收。

## [26.7.31v11] — 2026-07-31 · 🛡️ 统一工具策略、审批与审计

- Web、Telegram、飞书、Cron、Heartbeat 和 Subagent 统一执行全局与成员两层 Tool Policy，成员策略不能放宽全局边界；
- Policy JSON、Profile 或工具分组无效时默认移除全部工具，配置 API 同步拒绝保存无效策略；
- 策略改为在动态工具注册完成后统一执行，后续注册的环境、文件发送和子成员报告工具也无法绕过既有规则；
- 全部后台 Runner 接入同一个 Approval Broker；`ask` 命中但审批服务不可用时直接拒绝，不再静默执行；
- Pool 创建的全部 Runner 接入成员级工具审计，审批和工具审计写入失败会显式记录；
- 审批决定、超时和取消改为原子争抢唯一终态，修复重复审计及订阅取消与广播并发竞态；
- 审批 SSE 首帧和心跳携带权威待办快照，前端重连、批准或拒绝后重新同步，避免漏单和过期待办；
- 审批记录不再接受客户端伪造决策人，也不再把令牌片段写入审计日志。
...
- Public Chat 进一步以成员与 Web 渠道隔离 Worker，跨成员、跨渠道重连无法命中其他任务；
- 同一会话同一时间只接受一个任务，繁忙请求立即失败，不再清空或污染正在运行的事件代际；
- `StartGen` 收口到成功入队时执行，消除 Handler 与 Worker 重复清空回放缓冲区的竞态；
- Worker 对成功、错误和 panic 路径统一补齐终止事件，并在业务已发送终止事件时去重；
- 公共终止事件保留真实错误信息，同时增加全局 100 个、30 分钟活跃会话容量门禁；
- 活跃 Worker 不再被空闲定时器移除，SSE 取消订阅可中止大缓冲区回放，避免后台 goroutine 泄漏。

## [26.7.31v8] — 2026-07-31 · 🔄 Release E2E 矩阵撤回修复

- 修复 Linux、macOS 并行验证时由单个平台立即撤回 Release，导致另一平台读取不到最新版的竞态；
- 两个平台现在独立完成验证，仅在整个矩阵最终失败后由单独任务统一撤回；
- GitHub latest Release 等待上限由 48 秒提升为 5 分钟，并禁用 API 缓存，兼容跨地区传播延迟；
- `26.7.31v7` 本机正式在线验证和 Linux 远端验证通过，但因 macOS 节点读取到过期 latest 响应而自动转为草稿；本版本修复该门禁自身问题并重新发布。

## [26.7.31v7] — 2026-07-31 · 🛡️ 公共入口资源门禁与 SSRF 防护

- 公共 Web 渠道增加按来源的接口频率、模型执行频率和活跃会话上限；
- 增加全局公共模型任务及 SSE 连接并发上限，避免匿名请求耗尽服务资源；
- 单条公共消息限制为 16 KiB，单次后台模型执行增加 2 分钟硬超时；
- Public Chat 重连必须匹配原 Web 渠道，阻断跨渠道会话订阅；
- 新增统一 `pkg/netguard` 出站防护，拒绝本机、内网、链路本地、云元数据地址、危险 IPv6 和混合 DNS 解析；
- `web_fetch` 使用固定安全拨号目标并逐跳校验重定向，阻断 DNS 重绑定；浏览器初始导航同步启用 URL 校验；
- 增加公共限流、并发、会话、可信代理和 SSRF 地址分类回归测试；
- 修复唯一提交身份门禁在 `pipefail` 下可能漏检 `Co-authored-by` 的问题。

## [26.7.31v6] — 2026-07-31 · 🔐 Release E2E 严格模式

- 安装器增加禁止 GitHub 回退的严格模式，测试源或正式镜像异常时立即失败；
- 缺失或错误 `SHA256SUMS` 由警告升级为安装阻断，不再仅凭版本字符串接受二进制；
- 修复 `--no-service` 在 root 环境仍选择系统目录、系统配置目录写入失败及配置令牌文件权限过宽问题；
- Release E2E 增加成员创建、修改、删除、工作区文件读写和跨版本持久化验证；
- 增加错误校验和不替换旧二进制、不遗留 `.new` 文件的失败路径测试；
- 发布后直接等待并验证 `install.zyling.ai/latest`、`/install`、`/zyhive.sh` 和下载代理，不允许回退掩盖镜像故障。

## [26.7.31v5] — 2026-07-31 · 🧭 Release 安装更新全流程门禁

- 新增独立 Release E2E 模块，覆盖 SHA-256、全新安装、启动、鉴权、基础 API、管理界面、旧版更新、配置保持、备份和幂等；
- 正式发布前在临时 `HOME` 和本地 Release 仓库中完成隔离测试，不注册或操作用户系统服务；
- Release 创建后从 GitHub 正式资产再次验证在线安装，并从上一个正式版本真实更新；
- Linux 和 macOS 的 GitHub Release 工作流自动复测，失败时把 Release 转为草稿，停止作为公开最新版；
- 安装脚本增加指定版本、可替换下载源、`--no-service`、`--bind` 和非交互更新支持；
- 主分支 CI 增加发布安装更新门禁，正式发布脚本由 6 步扩展为 8 步。

## [26.7.31v4] — 2026-07-31 · 👤 唯一提交身份门禁

- 修正 `26.7.31v1` 至 `v3` 提交作者并移除自动加入的共同作者；
- 所有可达提交的 Author 和 Committer 统一为 `Zyling-ai <github@zyling.ai>`；
- 新增项目级 Cursor 永久规则，禁止使用本机身份或添加 `Co-authored-by`；
- 新增 CI 全历史提交身份检查；
- 发布脚本在构建前强制执行身份检查，失败时禁止创建 Release。

## [26.7.31v3] — 2026-07-31 · 🖼️ 媒体与审批短时凭证

- 媒体预览改为一次性 Artifact 凭证，不再把文件路径和管理令牌传给浏览器；
- `show_image` 只允许访问成员工作区，并返回不透明的媒体 URL；
- 审批 SSE 先通过受保护 REST 接口换取一分钟、单次连接凭证；
- 删除审批 SSE 对长期管理令牌查询参数的兼容；
- 增加媒体类型、路径、重放和审批凭证错误、过期、重放测试；
- 前端审批连接增加短时凭证交换、失败重连和自动化测试。

## [26.7.31v2] — 2026-07-31 · 🎫 一次性 Artifact 下载凭证

- 新增并发安全的 Artifact 凭证存储，仅保存凭证哈希和规范化文件路径；
- 下载链接改为随机 Artifact ID + 一次性短时凭证，不再暴露主机路径或长期管理令牌；
- 凭证默认十分钟过期、成功后立即失效，错误凭证不会破坏合法下载；
- `/api/download` 保留成员工作区约束作为第二层路径防护；
- Web 文件卡片和超大文件下载统一使用新凭证；
- 增加错误凭证、过期、重放、工作区外路径、URL 脱敏及竞态回归测试。

## [26.7.31v1] — 2026-07-31 · 🔒 质量恢复与安全边界

本版本首次正式发布此前主分支中的 `26.6.24v1`、`26.6.24v2` 开发成果，并集中恢复质量、发布和安全基线：

- 修复 AI 团队端到端测试夹具和 macOS 符号链接路径规范化问题；
- 管理 API、下载和媒体在令牌为空时改为拒绝访问；
- 下载、媒体及 `send_file` 限制在成员工作区，阻断任意主机路径；
- Public Chat 默认不暴露任何工具或成员环境变量；
- 飞书卡片回调增加签名、时间窗和重放验证；
- 修复 `gateway.bind` 未生效和默认管理令牌不安全问题；
- 在线升级和安装增加 SHA-256 校验、原子替换、备份及失败回滚；
- CI 增加前端测试、关键包竞态检测、嵌入 UI 校验和四平台构建；
- 发布脚本统一生成 Linux/macOS、AMD64/ARM64 产物及 `SHA256SUMS`；
- 增加开源商业边界和 2026—2027 全面优化路线文档。
