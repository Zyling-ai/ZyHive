...
func (h *chatHandler) execRunner(
	...
	var agentPolicy json.RawMessage
	if ag, ok := h.manager.Get(agentID); ok {
		agentPolicy = ag.ToolPolicyRaw
	}
	if err := toolRegistry.ConfigureGovernance(
		h.cfg.ToolPolicyRaw,
		agentPolicy,
		ApprovalBroker(),
		approvalTimeout(),
	); err != nil {
		// ConfigureGovernance has already removed every tool.
		log.Printf("[policy] blocked all tools for agent %s: %v", agentID, err)
	}
