...
> 文档状态：执行基线
> 制定日期：2026-07-31
> 最近复核：2026-08-02（当前稳定版 `26.8.2v1`）
> 适用周期：未来 12 个月
...
- 制定时正式版停在 `26.5.16v1` 且 CI 失败；截至 2026-08-02 已发布 `26.8.2v1` 并恢复 CI，完成 P0 持久化收口、核心旅程、完整备份恢复、Draft-first 可复现供应链、安装更新门禁、更新后健康回滚及统一远程管理连接；
- 核心 Runner、工具注册、API 路由和前端导航持续膨胀，缺少统一的任务运行模型；
- 没有持久化工作流检查点、完整 Trace、系统评测集、MCP/A2A 标准互操作；
- 高权限命令仍可在宿主机执行；后台进程虽已具备所有权、限额和完整回收，但实验性 sandbox 仍不等于强容器隔离；
- 前端约有 27 个 View，但只有 2 个前端测试文件，产品复杂度仍超过现有质量保障能力。
...
2. 把持续膨胀的内置工具集升级为“内置核心工具 + MCP 工具网关”；
...
- 当前公开稳定版为 `26.8.2v1`，已恢复主分支 CI、四平台产物、校验和、SBOM、签名、来源证明及严格安装更新与健康回滚全流程门禁；
...
- `main` 已启用管理员同样生效的分支保护，强制线性历史、分支最新状态及六项核心检查，禁止强推和删除；提交通过 PR 检查后以同一 SHA 快进，避免 GitHub 生成额外提交身份；
...
- 更新回滚已覆盖替换后的新进程启动阶段：`/healthz` 状态或版本不符合预期时，独立守护进程会原子恢复 `.bak` 并终止失败进程；Release E2E 同时验证成功确认和失联回滚。

结论：

> 发布链基线已经恢复，但这不等于产品可靠性建设完成。三个受保护提交已连续通过；下一阶段仍需补齐核心用户旅程 E2E、完整备份恢复、SBOM 和签名。

### 1.3 规模和复杂度

制定路线图时的仓库规模快照（前端测试数量已按 2026-08-01 更新；其余文件数量不作为持续事实）：
...
- README 声明支持多家模型供应商和丰富工具生态，并包含大量独立子系统；具体数量属于动态数据，不作为路线图长期事实。
...
- 已完成：质量修复与远端 CI、稳定版发布、四平台产物和 SHA-256、安装/升级门禁、后台进程与 Session 所有权、统一 Policy/Approval/Audit、公共入口限额、动态出站防护、Cron `at`/时区/重叠语义；
- 已完成：官网重构已部署并推送；E2E 已验证安装、升级、备份、错误校验和及新进程健康失败自动恢复；`main` 分支保护已启用；
- 部分完成：备份文件生成和可执行性已验证，完整数据恢复仍未覆盖；
- 已完成：前端开发、主 CI、staging 构建和正式发布门禁已迁移到 Node.js 22 LTS；
- 未完成：完整核心旅程 E2E、SBOM、签名和构建来源证明。

下列清单保留制定时原始范围，不表示每一项仍未完成；判断当前待办必须以上述状态和本文件末尾核对清单为准。
...
- [x] 模型流式、探测、健康、Key 测试、启动检查和 Embedding 动态地址统一执行公网限制；Ollama 仅开放显式精确回环端点，并随 `26.8.1v2` 发布；
- [x] 完成模型路径以外的剩余动态出站审计与收口，并随 `26.8.1v3` 发布；
- [x] 修复 Cron 一次性 `at`、任务时区、默认跳过重叠、原子注册、运行取消和成员所有权，并随 `26.8.1v4` 发布；
- [ ] 连续验证三个受保护 `main` 提交的远端 CI。

### 第 2 周：修复运行和持久化边界

- [x] 为 Session 双请求、断线重连和 Public Chat 增加关键回归测试；
- [x] 修复 Session 串流、Public Chat 终止事件和后台执行取消；
- [ ] 修复重复 Compaction 和配置原子提交；
- [ ] Labs 关闭时停止注册路由、工具和后台任务；
- [~] 收紧 systemd、文件权限、秘密脱敏和备份恢复路径；部分权限已收紧，完整恢复验证仍待完成；
- [x] 建立主分支保护和必要检查。

### 第 3 周：打通稳定产品旅程

- [~] 登录后的 REST、对话流、审批流、任务和媒体请求已统一远程服务器地址；配置递归合并仍待修复；
- [ ] 对齐 Provider 前后端契约；
- [ ] 修复成员创建部分失败；
- [ ] 统一成员级渠道；
- [x] 修复审批断线补拉并统一审批中心；
- [x] 停止未实现渠道的假成功；
- [x] 实现新进程健康检查失败自动回滚；
- [x] 从现行文档和官网移除或标明尚未实现的产品承诺。

### 第 4 周：全量验证并发布

- [x] 远端 CI 已修复；Node.js 22 LTS 已用于前端开发、CI、staging 构建与正式发布门禁，并通过第 2 个受保护提交验证；
- [~] 已执行全仓测试、关键 race、漏洞和前端构建；覆盖率与包体预算门禁未完成；
- [x] 已执行安装、升级、错误校验和、配置保持、完整备份破坏恢复及新进程健康回滚；
- [x] 已增加 Release E2E，并覆盖首次设置、渠道、任务、Goal/Cron、项目和工作区核心旅程；
- [x] 四平台产物、SHA-256、SPDX SBOM、Sigstore OIDC 签名和 GitHub 构建来源证明已完成；
- [x] 连续三个受保护 `main` 提交 CI 全绿；
- [x] 已发布质量恢复及后续稳定版本，当前为 `26.8.2v1`。
