...
- 当前公开稳定版为 `26.7.31v6`，已恢复主分支 CI、四平台产物、校验和及严格安装更新全流程门禁；
...
- [x] Public Chat 增加请求频率、模型频率、会话、任务并发、SSE 并发、消息大小和运行时限，并随 `26.7.31v7` 发布；
- [x] 建立统一出站 URL/DNS 防护，首批接入 `web_fetch` 和浏览器初始导航，并随 `26.7.31v7` 发布；
- [ ] 完成后台进程所有权、统一 Policy/Approval/Audit，以及浏览器跳转等剩余动态出站路径收口；
