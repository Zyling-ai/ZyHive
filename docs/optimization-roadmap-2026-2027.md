...
- 当前公开稳定版：`26.5.16v1`，发布日期 2026-05-16；
- 当前远端 `main`：2026-06-24 的未发布能力；
- 截至 2026-07-31：
  - 正式版约 11 周没有新发布；
  - 主分支约 5 周没有常规产品提交；
  - 用户感受到的“约 3 个月没更新”主要来自正式发布和官网同步中断。

### 1.2 当前质量记录

远端：

- 最新 GitHub Actions 仍失败；
- 自 2026-06-24 起已经连续 6 次 `main` CI 失败；
- 失败根因是 `pkg/aiteam/genesis_test` 测试夹具与工资默认值失配；
- 远端 CI 使用的部分 Actions 已出现 Node.js 20 运行时弃用提示；
- `main` 当前没有分支保护或 Ruleset，可绕过 CI 直接推送；
- 最新正式版只包含 `linux-amd64` 和 `darwin-arm64`，缺少另外两个宣称支持的平台、`SHA256SUMS`、SBOM 和签名/构建证明。

本地未提交工作区：

- `go test ./... -count=1 -timeout=10m`：通过；
- `go vet ./...`：通过；
- 前端 Vitest：1 个测试文件、4 个测试通过；
- `npm audit`：0 个已知漏洞；
- `git diff --check`：通过；
- Go 总语句覆盖率约 22.4%，39 个 Go 包中有 9 个没有测试；
- 前端只有 4 个用例，不能证明核心用户旅程；
- 已完成 CI、发布校验和、替换阶段回滚、网关绑定、随机令牌等第一轮修复，但尚未形成正式提交和 Release；
- 当前回滚只覆盖下载、校验和二进制替换阶段，尚未完成“新进程启动后健康检查失败自动恢复旧版本”的闭环。

结论：

> 远端代表“用户能获得的产品”，本地代表“尚未交付的修复”。在本地修复正式进入主分支之前，不能宣称质量问题已经解决。
...
- [ ] 下载进一步改为 Artifact ID + 一次性短时凭证；
- [ ] 完成后台进程所有权、统一 Policy/Approval/Audit、公共入口限流和 SSRF 防护；
