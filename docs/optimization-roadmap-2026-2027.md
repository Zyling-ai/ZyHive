...
> 文档状态：执行基线
> 制定日期：2026-07-31
> 最近复核：2026-08-01（当前稳定版 `26.8.1v8`）
> 适用周期：未来 12 个月
...
- 制定时正式版停在 `26.5.16v1` 且 CI 失败；截至 2026-08-01 已发布 `26.8.1v8` 并恢复 CI、安装更新门禁、更新后健康回滚及统一远程管理连接；
...
- 当前公开稳定版为 `26.8.1v8`，已恢复主分支 CI、四平台产物、校验和及严格安装更新与健康回滚全流程门禁；
...
- [~] 已执行安装、升级、错误校验和、配置保持、备份及新进程健康回滚；完整备份恢复未完成；
- [~] 已增加 Release E2E；完整 Stop-Ship 用户旅程仍未完成；
- [~] 四平台产物和 SHA-256 已完成；SBOM、签名和构建证明未完成；
- [x] 连续三个受保护 `main` 提交 CI 全绿；
- [x] 已发布质量恢复及后续稳定版本，当前为 `26.8.1v8`。
